...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
Game::Game( const tgrWindowProperties   &wprops,
            const std::filesystem::path &exeDir )
    : tgrApplication( wprops, true ), _exeDir( exeDir )
{
    glEnable( GL_BLEND );
...
void Game::Update()
{
    ...
    _shader->SetUniformMat4f(
        "u_P", _camera.GetProjectionMatrix( 1.0f, 0.01f, 100.0f ) );
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
bool Game::OnEvent( tgrEvent &evt )
{
    // _camera.OnEvent(evt);
    return true;
}
...
void Game::SetupGeometry()
{
    ...
    tgrVertexBufferLayout vblayout;
    ...
    _glMesh = std::make_unique<tgrMeshGL>( vertices, vblayout, conn );
}

// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
static bool ReadFileAsString( const std::filesystem::path &filePath,
                              std::string                 &output )
{
...
{
    static std::filesystem::path vertShaderFile =
        _exeDir / "shaders/v3n3t2_vert.glsl";
    static std::filesystem::path fragShaderFile =
        _exeDir / "shaders/fragment_custom.glsl";
    ...
    static auto vertLastModified =
        std::filesystem::last_write_time( vertShaderFile );
    static auto fragLastModified =
        std::filesystem::last_write_time( fragShaderFile );
    ...
    _shader = std::make_unique<tgrShader>( vertexSource, fragmentSource );
}
