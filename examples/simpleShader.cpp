...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
class TestsApplication : public tgrApplication
{
public:
    TestsApplication();
...
TestsApplication::TestsApplication()
    : tgrApplication( { 1920, 1080, "tinygraphics", true } )
{
    ...
    std::string fragmentShader =
        ...
        "    vec2 p = (2.0 * gl_FragCoord.xy - u_Size.xy) / min(u_Size.x, "
        "u_Size.y);\n"
        ...
        "    O = tanh(7.0 * exp(z.x - 4.0 - p.y * vec4(-1.0, 1.0, 2.0, 0.0)) / "
        "O);\n"
        "    return O;\n"
        "}\n"
...
void TestsApplication::Update()
{
    ...
    _shader->SetUniform2f(
        "u_Size", static_cast<float>( width ), static_cast<float>( height ) );
    double x, y;
    GetCursorPosition( x, y );
    _shader->SetUniform2f(
        "u_MousePos", static_cast<float>( x ), static_cast<float>( y ) );
