...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void tgrCamera::OnEvent( tgrEvent &evt )
{
    ...
    switch ( evtType )
    {
    case tgrEventType::MouseMoved:
    {
        auto &mouseEvt = static_cast<tgrMouseMoveEvent &>( evt );
        ...
        if ( dragging )
        {
            ...
            glm::vec3 cameraDirection =
                glm::normalize( GetPosition() - GetLookAt() );
            glm::vec3 cameraRight =
                glm::normalize( glm::cross( GetUpVec(), cameraDirection ) );
            glm::vec3 cameraUp = glm::cross( cameraDirection, cameraRight );
            SetUpVec( cameraUp );

            int    xPixelMove = mouseEvt.X() - prevDragX;
            double dthetaX =
                -1.0 * xPixelMove * ( 1.0 / 5.0 ) *
                ( M_PI / 180.0 ); // 1 deg rotation per 5 pixel move
            int    yPixelMove = mouseEvt.Y() - prevDragY;
            double dthetaY =
                -1.0 * yPixelMove * ( 1.0 / 5.0 ) *
                ( M_PI / 180.0 ); // 1 deg rotation per 5 pixel move

            glm::mat4 identity( 1.0f );
            glm::mat4 r = glm::rotate(
                identity, static_cast<float>( dthetaX ), cameraUp );
            r = glm::rotate( r, static_cast<float>( dthetaY ), cameraRight );
            glm::vec4 newDir = r * glm::vec4( cameraDirection, 1.0f );

            SetPosition( cor +
                         dist * glm::vec3( newDir[0], newDir[1], newDir[2] ) );
    ...
    case tgrEventType::MouseButtonPressed:
    {
        auto &mouseEvt = static_cast<tgrMousePressedEvent &>( evt );
        if ( mouseEvt.GetButton() == tgrMouseEvent::Button::MIDDLE )
        {
            buttonPressed = true;
            prevDragX     = mouseEvt.X();
            prevDragY     = mouseEvt.Y();
    ...
    case tgrEventType::MouseButtonReleased:
    {
        auto &mouseEvt = static_cast<tgrMouseReleasedEvent &>( evt );
        if ( mouseEvt.GetButton() == tgrMouseEvent::Button::MIDDLE )
        {
            buttonPressed = false;
        }
    ...
    case tgrEventType::MouseScrolled:
    {
        auto       &mouseEvt = static_cast<tgrMouseScrollEvent &>( evt );
    ...
    case tgrEventType::KeyPressed:
    {
        auto       &keyEvent    = static_cast<tgrKeyPressedEvent &>( evt );
    ...
    case tgrEventType::WindowResize:
    default:
        break;
    }
}
